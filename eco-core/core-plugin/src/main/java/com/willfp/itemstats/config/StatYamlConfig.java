...
public abstract class StatYamlConfig extends PluginDependent implements ValueGetter {
    /**
     ...
     */
    protected StatYamlConfig(@NotNull final String name,
                             @NotNull final Class<?> source) {
        super(AbstractEcoPlugin.getInstance());
        this.name = name;
        this.source = source;

        File basedir = new File(this.getPlugin().getDataFolder(), "stats/");
        if (!basedir.exists()) {
            basedir.mkdirs();
        }
    ...
    private void saveResource() {
        String resourcePath = "/stats/" + name + ".yml";
